...
local function EmoteDump()

	EmoteDump_SavedVariables = {}
	local sv = EmoteDump_SavedVariables
	...
	for i = 1, numEmotes do
		...
		sv[i]["slashName"] = slashName
		sv[i]["category"] = category
		sv[i]["emoteId"] = emoteId
		sv[i]["displayName"] = displayName
		
		if collectibleId ~= nil then
			local name, description, icon = GetCollectibleInfo(collectibleId)
			sv[i]["description"] = description
			sv[i]["icon"] = icon
			sv[i]["displayName"] = name
		end
	end
end

EVENT_MANAGER:RegisterForEvent("EmoteDump", EVENT_PLAYER_ACTIVATED, EmoteDump)